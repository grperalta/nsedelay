...
def print_order_convergence(list_error, mesh_size, tgrid_size):
    """
    Table for the mesh sizes, L2-error norms and order of convergences.
    """
    print(utils.Colors.UNDERLINE + ' '*80 + utils.Colors.ENDC)
    print("\t\t\t{}\n".format(list_error[0]))
    print("Time Stepsize\tMesh Size\tError\t\t\tOrder of Convergence")
    print(utils.Colors.UNDERLINE + ' '*80 + utils.Colors.ENDC)

    list_error = list_error[1:]
    for k in range(len(mesh_size)):
        if k == 0:
            print("{:.10f}\t{:.10f}\t{:.10e}".format(
                tgrid_size[k], mesh_size[k], list_error[k]))
        else:
            eoc = np.log(list_error[k] / list_error[k-1]) \
                / np.log(mesh_size[k] / mesh_size[k-1])
            print("{:.10f}\t{:.10f}\t{:.10e}\t{:.10e}".format(
                tgrid_size[k], mesh_size[k], list_error[k], eoc))
...
def get_ocp(NX, NT, femtype='p1_bubble'):
    """
...
def main(femtype='p1_bubble'):
    ...
    for k in range(len(LIST_NX)):
        # exact data and OCP classs
        exact_state, exact_adjoint, exact_control, ocp \
            = get_ocp(LIST_NX[k], LIST_NT[k], femtype=femtype)
        ocp.print()
        ...
        LIST_MESH_SIZE += [ocp.mesh.size()]
        LIST_TGRID_SIZE += [ocp.param.dt]
    ...
    for LIST in [LIST_L2_ERROR_STATE, LIST_L2_ERROR_ADJOINT,
        LIST_L2_ERROR_CONTROL]:
        print_order_convergence(LIST, LIST_MESH_SIZE, LIST_TGRID_SIZE)
